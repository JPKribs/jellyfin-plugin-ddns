...
public sealed class IonosProvider : DNSProviderBase
{
    ...
    /// <inheritdoc />
    public override string Label => "IONOS";

    /// <inheritdoc />
    public override string Hint => "Password is your IONOS API key in the form prefix.secret. Login is unused.";
    ...
    private static string? FindZoneId(string body, string host)
    {
        try
        {
            ...
            foreach (var zone in doc.RootElement.EnumerateArray())
            {
                ...
                if ((string.Equals(host, name, StringComparison.OrdinalIgnoreCase)
                    ...
                    && zone.TryGetProperty("id", out var idElement))
                {
        ...
        {
            // Body was not the expected JSON, so report no zone found.
        }
    ...
    private static string? FindRecordId(string body, string host, string type)
    {
        ...
        {
            // Body was not the expected JSON, so report no record found.
        }
