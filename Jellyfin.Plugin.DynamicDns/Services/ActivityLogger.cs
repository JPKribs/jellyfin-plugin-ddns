...
/// <summary>
/// Writes the plugin's notable events to Jellyfin's activity log, where administrators already look.
/// Entries are fire and forget and failures are swallowed, since a log entry must never break the
/// update run it documents. The static instance exists for the one chokepoint without DI access, the
...
public sealed class ActivityLogger
{
    ...
    /// <param name="type">The entry type, namespaced under <c>DynamicDNS.</c>.</param>
