...
OUTPUT_DIR="dist"
PROJECT_DIR="Jellyfin.Plugin.DynamicDns"
PROJECT_FILE="$PROJECT_DIR/Jellyfin.Plugin.DynamicDns.csproj"
...
validate_resources() {
    ...
    local missing_files=()
    local config_files=(
        "$PROJECT_DIR/Configuration/ddns_domains.html"
        "$PROJECT_DIR/Configuration/ddns_domains.js"
        "$PROJECT_DIR/Configuration/ddns_address.html"
        "$PROJECT_DIR/Configuration/ddns_address.js"
    )
...
# Main build process
main() {
    log "INFO" "Starting Dynamic DNS Plugin build"
    ...
    # Create ZIP package
    local zip_name="jellyfin-plugin-ddns-$VERSION.zip"
